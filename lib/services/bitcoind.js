...
Bitcoin.prototype.getAPIMethods = function() {
  var methods = [
    ...
    ['checkReferralCode', this, this.checkReferralCode, 1]
  ];
  return methods;
};
...
/*
 * Check if the referral/invite code someone gave you is valid
 * @param {String} referralCode
 * @param {Function} callback
 */
Bitcoin.prototype.checkReferralCode = function(referralCode, callback) {
  var self = this;

  if (typeof referralCode === 'string' || referralCode instanceof String) {
    var lowerCaseRefCode = referralCode.toLowerCase();

    this.client.checkReferralCode(lowerCaseRefCode, function(err, response) {
      if (err) {
        return callback(self._wrapRPCError(err));
      } else {
        callback(null, response.result);
