...
EmailService.prototype._getDataForTemplate = function (notification, recipient, cb) {
  ...
  if (data.amount) {
    try {
      ...
      data.amount = data.isInvite ? data.amount + ' invites' : Utils.formatAmount(+data.amount, unit) + ' ' +
        UNIT_LABELS[unit];
    } catch (ex) {
...
EmailService.prototype.sendEmail = function (notification, cb) {
  ...
  self.storage.fetchAndLockNotificationForEmails(Notification.fromObj(notification), function (err, isLocked) {
    if (err) {
          ...
          self.lock.runLocked('email-' + notification.id, cb, function (cb) {
            self.storage.fetchEmailByNotification(notification.id, function (err, email) {
              ...
              async.waterfall([
                ...
                function (next) {
                  self._readAndApplyTemplates(notification, emailType, recipientsList,
                    next);
                },
