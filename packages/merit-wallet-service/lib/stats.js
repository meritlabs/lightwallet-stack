...
Stats.prototype._getNewWallets = function(cb) {
  ...
  function updateStats(from, cb) {
    ...
    self.db.collection(storage.collections.WALLETS).mapReduce(map, reduce, opts, function(err, collection, stats) {
      return cb(err);
    });
  }
...
Stats.prototype._getTxProposals = function(cb) {
  ...
  function updateStats(from, cb) {
    ...
    self.db.collection(storage.collections.TXS).mapReduce(map, reduce, opts, function(err, collection, stats) {
      return cb(err);
    });
  }
