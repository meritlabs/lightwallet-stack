...
PayPro._createPayment = function(merchant_data, rawTx, refundAddr, amountQuanta) {
  ...
  var refund_outputs = this._getPayProRefundOutputs(refundAddr, amountQuanta);
  if (refund_outputs)
...
PayPro.send = function(opts, cb) {
  $.checkArgument(opts.merchant_data)
    ...
    .checkArgument(opts.amountQuanta);

  var payment = PayPro._createPayment(opts.merchant_data, opts.rawTx, opts.refundAddr, opts.amountQuanta);
