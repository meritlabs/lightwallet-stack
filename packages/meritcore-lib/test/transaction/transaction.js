...
var Input = meritcore.Transaction.Input;
var Output = meritcore.Transaction.Output;
...
describe('Transaction', function() {
  ...
  it('can serialize to a plain javascript object', function() {
    ...
    object.inputs[0].output.micros.should.equal(testAmount);
    ...
    object.outputs[0].micros.should.equal(testAmount - 10000);
  });
  ...
  it('toObject/fromObject with signatures and custom fee', function() {
    var tx = new Transaction()
      .from(simpleUtxoWith100000Micros)
      .to([{ address: toAddress, micros: 50000 }])
      .fee(15000)
  ...
  it('toObject/fromObject with p2sh signatures and custom fee', function() {
    var tx = new Transaction()
      .from(p2shUtxoWith1MRT, [p2shPublicKey1, p2shPublicKey2, p2shPublicKey3], 2)
      .to([{ address: toAddress, micros: 50000 }])
      .fee(15000)
      .change(changeAddress)
  ...
  it('fromObject with pay-to-public-key previous outputs', function() {
    ...
    tx.inputs[0].should.be.instanceof(meritcore.Transaction.Input.PublicKey);
    tx.inputs[0].output.micros.should.equal(5000000000);
  ...
  var simpleUtxoWith1000000Micros = {
    address: fromAddress,
    txId: 'a477af6b2667c29670467e4e0728b685ee07b240235771862318e29ddbe58458',
    outputIndex: 0,
    script: Script.buildPublicKeyHashOut(fromAddress).toString(),
  ...
  var anyoneCanSpendUTXO = JSON.parse(JSON.stringify(simpleUtxoWith100000Micros));
  ...
  describe('change address', function() {
    it('can calculate simply the output amount', function() {
      var transaction = new Transaction()
        .from(simpleUtxoWith1000000Micros)
        .to(toAddress, 500000)
        .change(changeAddress)
        .sign(privateKey);
      transaction.outputs.length.should.equal(2);
      transaction.outputs[1].micros.should.equal(400000);
    ...
    it('can recalculate the change amount', function() {
      ...
      transaction.getChangeOutput().micros.should.equal(50000);

      transaction = transaction.to(toAddress, 20000).sign(privateKey);

      transaction.outputs.length.should.equal(3);
      transaction.outputs[2].micros.should.equal(30000);
    ...
    it('fee can be set up manually', function() {
      var transaction = new Transaction()
        ...
        .sign(privateKey);
      transaction.outputs.length.should.equal(2);
      transaction.outputs[1].micros.should.equal(10000);
    });
    it('fee per kb can be set up manually', function() {
      var inputs = _.map(_.range(10), function(i) {
        var utxo = _.clone(simpleUtxoWith100000Micros);
        utxo.outputIndex = i;
        return utxo;
      });
      var transaction = new Transaction()
      ...
      transaction.outputs.length.should.equal(2);
      transaction.outputs[1].micros.should.equal(34000);
    });
    it('if micros are invalid', function() {
      var transaction = new Transaction()
        .from(simpleUtxoWith100000Micros)
        .to(toAddress, 99999)
        .change(changeAddress)
        .sign(privateKey);
      transaction.outputs[0]._micros = 100;
      transaction.outputs[0]._microsBN = new BN(101, 10);
      expect(function() {
        return transaction.serialize();
      }).to.throw(errors.Transaction.InvalidMicros);
    });
    ...
    it('getFee() returns the difference between inputs and outputs if no change address set', function() {
      var transaction = new Transaction().from(simpleUtxoWith100000Micros).to(toAddress, 1000);
  ...
  describe('serialization', function() {
    ...
    it('can avoid checked serialize', function() {
      var transaction = new Transaction().from(simpleUtxoWith1MRT).to(fromAddress, 1);
      expect(function() {
        return transaction.serialize();
      }).to.throw();
      expect(function() {
  ...
  describe('checked serialize', function() {
    it('fails if no change address was set', function() {
      var transaction = new Transaction().from(simpleUtxoWith1MRT).to(toAddress, 1);
      expect(function() {
        return transaction.serialize();
    ...
    it('checks output amount before fee errors', function() {
      var transaction = new Transaction();
      transaction.from(simpleUtxoWith1MRT);
      transaction
    ...
    describe('skipping checks', function() {
      ...
      it(
        'can skip the check that avoids spending more MRT than the inputs for a transaction',
        buildSkipTest(
          function(transaction) {
            return transaction
              .to(toAddress, 10000000000000)
  ...
  describe('#verify', function() {
    it('not if _micros and _microsBN have different values', function() {
      var tx = new Transaction()
        .from({
          txId: testPrevTx,
          outputIndex: 0,
      ...
      tx.outputs[0]._micros = 100;
      tx.outputs[0]._microsBN = new BN('fffffffffffffff', 16);
      var verify = tx.verify();
      verify.should.equal('transaction txout 0 micros is invalid');
    });

    it('not if _micros is negative', function() {
      var tx = new Transaction()
        .from({
          txId: testPrevTx,
          outputIndex: 0,
      ...
      tx.outputs[0]._micros = -100;
      tx.outputs[0]._microsBN = new BN(-100, 10);
      var verify = tx.verify();
      verify.should.equal('transaction txout 0 micros is invalid');
    });
  ...
  describe('serialization of inputs', function() {
    it('can serialize and deserialize a P2PKH input', function() {
      var transaction = new Transaction().from(simpleUtxoWith1MRT);
  ...
  describe('checks on adding inputs', function() {
    ...
    it('fails if no micro amount is provided', function() {
    ...
    it('will add an empty script if not supplied', function() {
      ...
      transaction.inputs[0].output.script.should.be.instanceof(meritcore.Script);
  ...
  describe('removeInput and removeOutput', function() {
    it('can remove an input by index', function() {
      ...
      transaction.inputAmount.should.equal(simpleUtxoWith1MRT.micros);
    ...
    it('can remove an input by transaction id', function() {
      ...
      transaction.inputs.length.should.equal(1);
      transaction.inputAmount.should.equal(simpleUtxoWith1MRT.micros);
      transaction.removeInput(simpleUtxoWith1MRT.txId, simpleUtxoWith1MRT.outputIndex);
      transaction.inputs.length.should.equal(0);
      transaction.inputAmount.should.equal(0);
    });
    ...
    it('an output can be removed by index', function() {
      var transaction = new Transaction().to([
        { address: toAddress, micros: 40000000 },
        { address: toAddress, micros: 40000000 },
      ]);
      transaction.outputs.length.should.equal(2);
  ...
  describe('handling the nLockTime', function() {
    ...
    it('has a non-max sequenceNumber for effective date locktime tx', function() {
      var transaction = new Transaction().from(simpleUtxoWith1MRT).lockUntilDate(date);
    ...
    it('has a non-max sequenceNumber for effective blockheight locktime tx', function() {
      var transaction = new Transaction().from(simpleUtxoWith1MRT).lockUntilBlockHeight(blockHeight);
      transaction.inputs[0].sequenceNumber.should.equal(Transaction.Input.DEFAULT_LOCKTIME_SEQNUMBER);
    });
  ...
  describe('inputAmount + outputAmount', function() {
    it('returns correct values for simple transaction', function() {
      var transaction = new Transaction().from(simpleUtxoWith1MRT).to(toAddress, 40000000);
  ...
  describe('output ordering', function() {
    ...
    beforeEach(function() {
      transaction = new Transaction()
        .from(simpleUtxoWith1MRT)
        .to([{ address: toAddress, micros: tenth }, { address: toAddress, micros: fourth }])
        .to(toAddress, half)
        .change(changeAddress);
  ...
  describe('clearOutputs', function() {
    it('removes all outputs and maintains the transaction in order', function() {
      var tx = new Transaction()
        .from(simpleUtxoWith1MRT)
        .to(toAddress, tenth)
        .to([{ address: toAddress, micros: fourth }, { address: toAddress, micros: half }])
        .change(changeAddress);
      tx.clearOutputs();
      tx.outputs.length.should.equal(1);
      tx.to(toAddress, tenth);
      tx.outputs.length.should.equal(2);
      tx.outputs[0].micros.should.equal(10000000);
      tx.outputs[0].script
        .toAddress()
        .toString()
        .should.equal(toAddress);
      tx.outputs[1].micros.should.equal(89900000);
      tx.outputs[1].script
        .toAddress()
        .toString()
  ...
  describe('BIP69 Sorting', function() {
    ...
    it('sorts outputs correctly', function() {
      ...
      tx.sort();
      tx.outputs[0].micros.should.equal(1);
      tx.outputs[1].micros.should.equal(2);
      tx.outputs[2].micros.should.equal(2);
    ...
    describe('meritjs fixtures', function() {
        ...
        it(inputSet.description, function() {
          ...
          inputSet.inputs = inputSet.inputs.map(function(input) {
            var input = new Input({
              ...
              script: new Script(),
              output: new Output({ script: new Script(), micros: 0 }),
            });
    ...
    describe('#isRBF', function() {
      ...
      it('determine opt-out with 0xfffffffe', function() {
        var tx = new Transaction()
          .from(simpleUtxoWith1MRT)
          .from(simpleUtxoWith100000Micros)
          .to([{ address: toAddress, micros: 50000 + 1e8 }])
          .fee(15000)
          .change(changeAddress)
          .sign(privateKey);
...
var unsupportedTxObj =
  '{"version":1,"inputs":[{"prevTxId":"a477af6b2667c29670467e4e0728b685ee07b240235771862318e29ddbe58458","outputIndex":0,"sequenceNumber":4294967295,"script":"OP_1","output":{"micros":1020000,"script":"OP_1 OP_ADD OP_2 OP_EQUAL"}}],"outputs":[{"micros":1010000,"script":"OP_DUP OP_HASH160 20 0x7821c0a3768aa9d1a37e16cf76002aef5373f1a8 OP_EQUALVERIFY OP_CHECKSIG"}],"nLockTime":0}';
