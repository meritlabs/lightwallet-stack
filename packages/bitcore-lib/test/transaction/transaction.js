...
describe('Transaction', function() {
  ...
  it('can serialize to a plain javascript object', function() {
    ...
    object.inputs[0].output.quanta.should.equal(testAmount);
    ...
    object.outputs[0].quanta.should.equal(testAmount - 10000);
  });
  ...
  it('toObject/fromObject with signatures and custom fee', function() {
    var tx = new Transaction()
      .from(simpleUtxoWith100000Quanta)
      .to([{address: toAddress, quanta: 50000}])
      .fee(15000)
  ...
  it('fromObject with pay-to-public-key previous outputs', function() {
    ...
    tx.inputs[0].output.quanta.should.equal(5000000000);
  ...
  var simpleUtxoWith100000Quanta = {
    address: fromAddress,
  ...
  var simpleUtxoWith1000000Quanta = {
    address: fromAddress,
    txId: 'a477af6b2667c29670467e4e0728b685ee07b240235771862318e29ddbe58458',
    outputIndex: 0,
    script: Script.buildPublicKeyHashOut(fromAddress).toString(),
  ...
  var anyoneCanSpendUTXO = JSON.parse(JSON.stringify(simpleUtxoWith100000Quanta));
  ...
  describe('change address', function() {
    it('can calculate simply the output amount', function() {
      var transaction = new Transaction()
        .from(simpleUtxoWith1000000Quanta)
        .to(toAddress, 500000)
        .change(changeAddress)
        .sign(privateKey);
      transaction.outputs.length.should.equal(2);
      transaction.outputs[1].quanta.should.equal(400000);
    ...
    it('can recalculate the change amount', function() {
      ...
      transaction.getChangeOutput().quanta.should.equal(50000);
      ...
      transaction.outputs[2].quanta.should.equal(30000);
    ...
    it('fee can be set up manually', function() {
      var transaction = new Transaction()
        ...
        .sign(privateKey);
      transaction.outputs.length.should.equal(2);
      transaction.outputs[1].quanta.should.equal(10000);
    });
    it('fee per kb can be set up manually', function() {
      var inputs = _.map(_.range(10), function(i) {
        var utxo = _.clone(simpleUtxoWith100000Quanta);
        utxo.outputIndex = i;
        return utxo;
      });
      var transaction = new Transaction()
      ...
      transaction.outputs.length.should.equal(2);
      transaction.outputs[1].quanta.should.equal(34000);
    });
    it('if quanta are invalid', function() {
      var transaction = new Transaction()
        .from(simpleUtxoWith100000Quanta)
        .to(toAddress, 99999)
        .change(changeAddress)
        .sign(privateKey);
      transaction.outputs[0]._quanta = 100;
      transaction.outputs[0]._quantaBN = new BN(101, 10);
      expect(function() {
        return transaction.serialize();
      }).to.throw(errors.Transaction.InvalidQuanta);
    });
  ...
  describe('#verify', function() {
    ...
    it('not if _quanta and _quantaBN have different values', function() {
      var tx = new Transaction()
        .from({
          'txId': testPrevTx,
          'outputIndex': 0,
      ...
      tx.outputs[0]._quanta = 100;
      tx.outputs[0]._quantaBN = new BN('fffffffffffffff', 16);
      var verify = tx.verify();
      verify.should.equal('transaction txout 0 quanta is invalid');
    });

    it('not if _quanta is negative', function() {
      var tx = new Transaction()
        .from({
          'txId': testPrevTx,
          'outputIndex': 0,
      ...
      tx.outputs[0]._quanta = -100;
      tx.outputs[0]._quantaBN = new BN(-100, 10);
      var verify = tx.verify();
      verify.should.equal('transaction txout 0 quanta is invalid');
    });
  ...
  describe('removeInput and removeOutput', function() {
    it('can remove an input by index', function() {
      ...
      transaction.inputAmount.should.equal(simpleUtxoWith1MRT.quanta);
    ...
    it('an output can be removed by index', function() {
      var transaction = new Transaction()
        .to([
          {address: toAddress, quanta: 40000000},
          {address: toAddress, quanta: 40000000}
        ])
      transaction.outputs.length.should.equal(2);
  ...
  describe('output ordering', function() {
    ...
    beforeEach(function() {
      transaction = new Transaction()
        .from(simpleUtxoWith1MRT)
        .to([
          {address: toAddress, quanta: tenth},
          {address: toAddress, quanta: fourth}
        ])
        .to(toAddress, half)
        .change(changeAddress);
  ...
  describe('clearOutputs', function() {
    ...
    it('removes all outputs and maintains the transaction in order', function() {
      ...
      tx.outputs[0].quanta.should.equal(10000000);
      tx.outputs[0].script.toAddress().toString().should.equal(toAddress);
      tx.outputs[1].quanta.should.equal(89900000);
  ...
  describe('BIP69 Sorting', function() {
    ...
    it('sorts outputs correctly', function() {
      ...
      tx.sort();
      tx.outputs[0].quanta.should.equal(1);
      tx.outputs[1].quanta.should.equal(2);
      tx.outputs[2].quanta.should.equal(2);
        ...
        it(inputSet.description, function() {
          ...
          inputSet.inputs = inputSet.inputs.map(function(input) {
            var input = new Input({
              ...
              script: new Script(),
              output: new Output({ script: new Script(), quanta: 0 })
            });
    ...
    describe('#isRBF', function() {
      ...
      it('determine opt-out with 0xfffffffe', function() {
        var tx = new Transaction()
          .from(simpleUtxoWith1MRT)
          .from(simpleUtxoWith100000Quanta)
          .to([{address: toAddress, quanta: 50000 + 1e8}])
          .fee(15000)
          .change(changeAddress)
          .sign(privateKey);
...
var unsupportedTxObj = '{"version":1,"inputs":[{"prevTxId":"a477af6b2667c29670467e4e0728b685ee07b240235771862318e29ddbe58458","outputIndex":0,"sequenceNumber":4294967295,"script":"OP_1","output":{"quanta":1020000,"script":"OP_1 OP_ADD OP_2 OP_EQUAL"}}],"outputs":[{"quanta":1010000,"script":"OP_DUP OP_HASH160 20 0x7821c0a3768aa9d1a37e16cf76002aef5373f1a8 OP_EQUALVERIFY OP_CHECKSIG"}],"nLockTime":0}';
