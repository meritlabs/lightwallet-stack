  ...
  switch (address.type) {
    case Constants.SCRIPT_TYPES.P2SH:
      scriptPubKey = address.publicKeys
        ? Meritcore.Script.buildMultisigOut(address.publicKeys, m).toScriptHashOut()
        : '';
      break;
    case Constants.SCRIPT_TYPES.P2PKH:
      scriptPubKey = Meritcore.Script.buildPublicKeyHashOut(address.address);
...
describe('client API', function() {
    ...
    it('Should return all main addresses', function(done) {
      clients[0].getMainAddresses(
        ...
        function(err, addr) {
          ...
          addr.length.should.equal(1); // for singleAddress = true
          done();
        },
      );
    });
  ...
  /*
   * skip legacy import as it does not support beacon and share codes
  ...
  describe('Sweep paper wallet', function() {
    ...
    it('should get balance from single private key', function(done) {
      ...
      helpers.createAndJoinWallet(clients, 1, 1, function() {
        ...
        clients[0].getBalanceFromPrivateKey('5KjBgBiadWGhjWmLN1v4kcEZqWSZFqzgv7cSUuZNJg4tD82c4xp', function(
          err,
          balance,
        ) {
          should.not.exist(err);
      ...
      helpers.createAndJoinWallet(clients, 1, 1, function() {
        ...
        clients[0].buildTxFromPrivateKey(
          ...
          function(err, tx) {
            ...
            var script = new Meritcore.Script.buildPublicKeyHashOut(
              Meritcore.Address.fromString('1GG3JQikGC7wxstyavUBDoCJ66bWLLENZC'),
            );
