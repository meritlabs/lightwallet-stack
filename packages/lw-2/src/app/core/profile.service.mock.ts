...
export class ProfileServiceMock  {
  ...
  getWallets():Promise<Array<Wallet>> {
    ...
    return new Promise((rs,rj) => {
      rs([
        ...
        new WalletMock({id: '7', color: 'darkcyan', name: 'Processing wallet', status: {totalBalanceStr: '10 bits', totalBalanceSatoshis: 10, spendableAmount: 0}}),
  ...
  public getTxps(opts: any): Promise<any> {
    return new Promise((resolve, reject) => {
      ...
      resolve({
        txps: [
          {message: 'txps message', amountStr: '10 satoshis', createdOn: ((new Date()).getTime()/1000 - 1234), action: 'sent'},
          {toAddress: '123', amountStr: '10 satoshis', action: 'invalid'},
          {merchant: {domain: 'mock domain', pr: {ca: true}}, amountStr: '10 satoshis', action: 'sent'}
        ],
        txpsN: 4
      });
